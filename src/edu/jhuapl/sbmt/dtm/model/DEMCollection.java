...
import edu.jhuapl.sbmt.dtm.service.events.HideDEMEvent;
import edu.jhuapl.sbmt.dtm.service.events.ShowDEMEvent;
...
public class DEMCollection extends AbstractModel implements PropertyChangeListener, EventListener
{
    ...
    public DEMKey getDEMKeyFromInfo(DEMKey info)
    {
        for (DEM dem : demToActorsMap.keySet())
        {
            if (dem.getKey().demfilename.equals(info.demfilename))
                return dem.getKey();
        }
    ...
    public void addDEM(DEMKey key) //throws IOException
    {
...
//        }
    ...
    public void saveDEM(DEMKey key) throws IOException
    {
        ...
        if(key.demfilename.toLowerCase().endsWith(".fit") || key.demfilename.toLowerCase().endsWith(".fits"))
        {
            ...
            FileUtil.copyFile(key.demfilename,  newFilepath);
            // Change demInfo.demfilename to the new location of the file
            key.demfilename = newFilepath;
        }
        else
        {
