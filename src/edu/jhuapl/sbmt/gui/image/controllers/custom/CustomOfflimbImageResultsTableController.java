...
//        }
    ...
    private void updateBodyPositions(int row, Runnable completionBlock)
    {
    	...
    	try
		{
			...
			List<SmallBodyModel> bodies = modelManager.getModels(ModelNames.SMALL_BODY).stream().map(body -> { return (SmallBodyModel)body; }).toList();
    		Image image = SbmtModelFactory.createImage(key, bodies, false);
