...
public class ImageResultsTableController
{
    ...
    public ImageResultsTableController(ImagingInstrument instrument, ImageCollection imageCollection, ImageSearchModel model, Renderer renderer, SbmtInfoWindowManager infoPanelManager, SbmtSpectrumWindowManager spectrumPanelManager)
    {
        ...
        PerspectiveImageBoundaryCollection boundaries = (PerspectiveImageBoundaryCollection)modelManager.getModel(model.getImageBoundaryCollectionModelName());
...
//        jScrollPane4.setViewportView(resultList);
        ...
        imageResultsTableView.getResultList().getModel().addTableModelListener(new TableModelListener()
        {
            ...
            public void tableChanged(TableModelEvent e)
            {
                ...
                else if (e.getColumn() == imageResultsTableView.getBndrColumnIndex())
                {
                    ...
                    try
                    {
                        PerspectiveImageBoundaryCollection boundaries = (PerspectiveImageBoundaryCollection)modelManager.getModel(imageSearchModel.getImageBoundaryCollectionModelName());
        ...
        imageResultsTableView.getResultList().setDefaultRenderer(String.class, new StringRenderer());
        ...
        imageResultsTableView.getResultList().getColumn(imageResultsTableView.getMapColumnIndex()).setResizable(true);
        imageResultsTableView.getResultList().getColumn(imageResultsTableView.getShowFootprintColumnIndex()).setResizable(true);
        imageResultsTableView.getResultList().getColumn(imageResultsTableView.getFrusColumnIndex()).setResizable(true);
        imageResultsTableView.getResultList().getColumn(imageResultsTableView.getBndrColumnIndex()).setResizable(true);
        imageResultsTableView.addPropertyChangeListener(new PropertyChangeListener()
        {
...
//                        ImageKey key = new ImageKey(name.substring(0, name.length()-4), sourceOfLastQuery, instrument);
                        ...
                        if (imageSearchModel.getImageCollection().containsImage(key))
                        {
                            resultList.setValueAt(true, i, imageResultsTableView.getMapColumnIndex());
                            PerspectiveImage image = (PerspectiveImage) imageSearchModel.getImageCollection().getImage(key);
...
//            ImageKey key = new ImageKey(name.substring(0, name.length()-4), sourceOfLastQuery, instrument);
            ...
            imageResultsTableView.getSaveSelectedImageListButton().setEnabled(resultList.getSelectedRowCount() > 0);
    ...
    public class ImagesTableModel extends DefaultTableModel
    {
        ...
        public boolean isCellEditable(int row, int column)
        {
            ...
            if (column == imageResultsTableView.getShowFootprintColumnIndex() || column == imageResultsTableView.getFrusColumnIndex())
            {
                String name = imageRawResults.get(row).get(0);
                ImageKey key = imageSearchModel.createImageKey(name.substring(0, name.length()-4), imageSearchModel.getImageSourceOfLastQuery(), imageSearchModel.getInstrument());
                return imageSearchModel.getImageCollection().containsImage(key);
            }
            else
            {
