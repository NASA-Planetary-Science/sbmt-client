...
//        int szMax=Math.max(resolution[0], resolution[1]);
    ...
    private vtkPolyData getOffLimbPlane(PerspectiveImage img, double offLimbFootprintDepth)
    {
        ...
        String offLimbImageDataFileName = new File(new File(img.getFitFileFullPath()).getParent()).getParent() + File.separator + "support" + File.separator + img.getKey().getSource().name() + File.separator + FilenameUtils.getBaseName(img.getFitFileFullPath()) + "_" + img.getSmallBodyModel().getModelResolution() + "_offLimbImageData.vtk.gz";
