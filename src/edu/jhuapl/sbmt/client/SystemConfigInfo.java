...
public class SystemConfigInfo implements MetadataManager
{
...
//	String[] modelLabel;
	...
	public SystemConfigInfo(String systemName, String[] uniqueNames, boolean publishedDataOnly)
	{
//		this.type = config[0].type;
//        this.population = config[0].population;
//        this.system = config[0].system;
//        List.of(config).stream().map( o -> o.body).collect(Collectors.toList()).toArray(this.body);
//		this.dataUsed = config[0].dataUsed;
//		List.of(config).stream().map( o -> o.getAuthor()).collect(Collectors.toList()).toArray(this.author);
//		List.of(config).stream().map( o -> o.getShapeModelName()).collect(Collectors.toList()).toArray(this.shapeModelName);
		this.system = ShapeModelBody.valueFor(systemName);
		this.uniqueSystemName = systemName;
		this.uniqueNames = uniqueNames;
//		List.of(config).stream().map( o -> o.getUniqueName()).collect(Collectors.toList()).toArray(this.uniqueName);
//		List.of(config).stream().map( o -> o.getVersion()).collect(Collectors.toList()).toArray(this.version);
//		List.of(config).stream().map( o -> o.getModelLabel()).collect(Collectors.toList()).toArray(this.modelLabel);
//		List.of(config).stream().map( o -> o.presentInMissions).collect(Collectors.toList()).toArray(this.presentInVersion);
//		List.of(config).stream().map( o -> o.defaultForMissions).collect(Collectors.toList()).toArray(this.defaultFor);
...
//	Key<String> shapeModelNameKey = Key.of("shapeModelName");
	Key<String> uniqueSystemNameKey = Key.of("uniqueSystemName");
...
//        configMetadata.put(shapeModelNameKey, shapeModelName);
        configMetadata.put(uniqueNameKey, uniqueNames);
...
//		shapeModelName = source.get(shapeModelNameKey);
		uniqueNames = source.get(uniqueNameKey);
...
//		{
            ...
            if (SmallBodyViewConfig.getDefaultModelName() == null)
            {
                for (SbmtMultiMissionTool.Mission defaultMission : defaultFor)
                {
                    if (defaultMission == SbmtMultiMissionTool.getMission())
                    {
                        SmallBodyViewConfig.setDefaultModelName(uniqueSystemName);
                        break;
                    }
                }
            }
