...
public class DeimosQuery extends QueryBase
{
    ...
    {
        result.set(0, "/THOMAS/DEIMOS/IMAGING/images/" + result.get(0));
    }
        ...
        //if ("".equals(type))
        {
            if (searchString != null)
            {
                try
                {
                    ...
                    results = doQuery("searchdeimosimages_id.php", constructUrlArguments(args));
                }
            ...
            boolean phobos2 = false;
            boolean vikingOrbiter1A = userDefined.get(0);
            boolean vikingOrbiter1B = userDefined.get(1);
            boolean vikingOrbiter2A = userDefined.get(2);
            boolean vikingOrbiter2B = userDefined.get(3);
            boolean mexHrsc = userDefined.get(4);
            boolean mroHirise = false;
            boolean mgsMoc = false;
            ...
            try
            {
                ...
                // Add 3 to the filters since the first 3 are meant for VSK
                // which the DEIMOS gui does not contain checkboxes for.
                // This is a bad hack. In fact the entire query package needs to
                // be redesigned since there's too much duplication.
                for (int i=0; i<filters.size(); ++i)
                    filters.set(i, filters.get(i) + 3);
                ...
                results = doQuery("searchdeimosimages.php", constructUrlArguments(args));
