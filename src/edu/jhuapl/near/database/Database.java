...
/**
 * This class represents a database storing information about all the msi, nis,
 * and nlr data. It also provides functions for querying the database.
 * 
 * @author kahneg1
 * 
 */
public class Database {
	...
	{
		InputStream is = getClass().getResourceAsStream("/edu/jhuapl/near/data/msidatabase.txt");
	 ...
	 * @param endDate
	 */
	public ArrayList<String> runQuery(LocalDateTime startDate, LocalDateTime stopDate) 
	{
		ArrayList<String> matchedImages = new ArrayList<String>();
		
		for (ImageInfo info: images)
		{
			if (containsDate(startDate, stopDate, info))
