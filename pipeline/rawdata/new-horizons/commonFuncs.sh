...
createSymbolicLink() {
  (
    ...
    if test "x$src" = x; then
      echo "Cannot create link: source file/directory is blank." >> $log 2>&1
      exit 1
    fi
    if test "x$dest" = x; then
      echo "Cannot create link: destination file/directory is blank." >> $log 2>&1
      exit 1
    fi

    linkName=`echo $dest | sed 's:.*/::'`
    destDir=`echo $dest | sed -n "s:$linkName$::p"`
    if test "x$destDir" != x; then
      cd $destDir >> $log 2>&1
      if test $? -ne 0; then
        echo "Cannot create link: unable to cd $destDir" >> $log 2>&1
        exit 1
      fi
    fi

    if test ! -e $src; then
      echo "Cannot create link: source file/directory $src does not exist" >> $log 2>&1
      exit 1
    fi

    if test -e $linkName  -a "`ls -iL $linkName 2> /dev/null | sed 's: .*::'`" = "`ls -iL $dest 2> /dev/null | sed 's: .*::'`"; then
      echo "Already linked: $dest -> $src" >> $log 2>&1
      exit 0
    elif test -e $linkName; then
      echo "Cannot create link: destination $dest already exists" >> $log 2>&1
      exit 1
    fi

    echo "ln -s $src $dest" >> $log 2>&1
    ln -s $src $dest  >> $log 2>&1
    if test $? -ne 0; then
      echo "Cannot create link" >> $log 2>&1
      exit 1
    fi
  )
  if test $? -ne 0; then exit 1; fi
