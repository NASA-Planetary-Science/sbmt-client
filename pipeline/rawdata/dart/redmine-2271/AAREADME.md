#About Scripts in This Folder
--------------------------------------------------------------------------------

This version of the data processing scripts was derived from legacy scripts
that were significantly refactored and augmented in order to process the initial
round of DART simulated model/image deliveries (20 models). This collection
of simulated models, images and SPICE kernels is known to the DART team as
20200629-v01. In redmine there was belatedly added a parent issue #2271 that
described the ~60 children issue associated with this delivery.

The main feature added in this version was support for generating INFO files
directly from SPICE files as part of the script for processing deliveries.
Significantly, images still reside under each model with these scripts, as
had always been the case. For these models, this was only a factor of 2 bloat,
because each simulation (e.g., ideal-impact-1) had two body models that
contained copies of the same images. (Note that Osiris-REx and Hayabusa-2
images were centralized using a symbolic link on the server. As far as the
client was concerned, the images remained the property of each model). 
