...
int main(int argc, char** argv)
{
    if (argc < 11)
    {
        cerr << "Usage: create_info_files <kernelfiles> <body> <bodyFrame> <sc> <instrframe> <fitstimekeyword> <inputfilelist> <infofilefolder> <outputfilelist> <errorLogFile>" << endl;
        return 1;
    }
	...
	getFov(et, scid, body, bodyFrame, instr, boredir, updir, frustum);
        if (failed_c())
	{
	    errorOut << "Unable to resolve SPICE state for image file; see rawdata2processed log " << fitfiles[i] << endl;
            continue;
	}
