 ...
 *
 */
public class MSIImage_old extends Model
{
	...
	{
		static private WeakHashMap<MSIImage_old, Object> images = 
			new WeakHashMap<MSIImage_old, Object>();
		
		static public MSIImage_old createImage(String name) throws FitsException, IOException
		{
			for (MSIImage_old image : images.keySet())
			{
				if (image.getServerPath().equals(name))
					return image;
			}

			MSIImage_old image = new MSIImage_old(name);
	...
	public MSIImage_old(String filename) throws FitsException, IOException
	{
	...
	public MSIImage_old(File fitFile) throws FitsException, IOException
	{
		this.initialize(fitFile);
	}
    ...
    public ArrayList<vtkProp> getProps()
    {
        ...
        int numTexturesWidth = (int)Math.ceil((double)(MSIImage_old.IMAGE_WIDTH-1) / (double)(TEXTURE_SIZE-1));
        int numTexturesHeight = (int)Math.ceil((double)(MSIImage_old.IMAGE_HEIGHT-1) / (double)(TEXTURE_SIZE-1));
        ...
        for (int i=0; i<numTexturesHeight; ++i)
        	for (int j=0; j<numTexturesWidth; ++j)
        	{
                ...
                if (i==numTexturesHeight-1)
                	maxM = (MSIImage_old.IMAGE_HEIGHT) - i*(TEXTURE_SIZE-1);
                int maxN = TEXTURE_SIZE;
                if (j==numTexturesWidth-1)
                	maxN = (MSIImage_old.IMAGE_WIDTH) - j*(TEXTURE_SIZE-1);
...
//	}
	...
	public vtkPolyData generateImageBorder()
	{
        ...
        //points.SetNumberOfPoints(maxM*maxN);
        indices = new int[MSIImage_old.IMAGE_HEIGHT][MSIImage_old.IMAGE_WIDTH];
        
        // First add points to the vtkPoints array
        for (int m=0; m<MSIImage_old.IMAGE_HEIGHT; ++m)
			for (int n=0; n<MSIImage_old.IMAGE_WIDTH; ++n)
			{
				x = getX(m, n);
				y = getY(m, n);
				z = getZ(m, n);
        ...
        // Now add connectivity information
        for (int m=1; m<MSIImage_old.IMAGE_HEIGHT; ++m)
			for (int n=1; n<MSIImage_old.IMAGE_WIDTH; ++n)
			{
				// Get the indices of the 4 corners of the rectangle to the upper left
				i0 = indices[m-1][n-1];
				i1 = indices[m][n-1];
...
//    }
    ...
    {
    	if (x <= MSIImage_old.PDS_NA || y <= MSIImage_old.PDS_NA || z <= MSIImage_old.PDS_NA)
    		return false;
    	else
    		return true;
    }
