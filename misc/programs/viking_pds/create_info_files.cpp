...
void getEt(const string& labelfile,
           string& startutc,
           double& startet,
           string& stoputc,
           double& stopet)
{
    ...
    if (fin.is_open())
    {
        ...
        while(true)
        {
            ...
            if (str == "IMAGE_TIME")
            {
                ...
                cout << labelfile << " IMAGE_TIME " << startutc << " " << startet << std::endl;
                break;
            }
        }
    }
    ...
    {
        cerr << "Error: Unable to open file '" << labelfile << "'" << endl;
        exit(1);
    }
...
*/
void getScOrientation(double et, string body, string sc, string frame, double scposb[3], double boredir[3], double updir[3],
                      double frustum[12])
{
    double lt;
    double i2bmat[3][3];
    ...
    xo=-tan(0.0316815603057/2.0);
    yo=-tan(0.0277876855117/2.0);
...
void getCameraAndSc(string labelfile, string& camera, string& sc)
{
    ifstream fin(labelfile.c_str());
    ...
    if (fin.is_open())
    {
        ...
        while(true)
        {
            ...
            if (str == "SPACECRAFT_NAME")
            {
                fin >> str; // the equals character
                fin >> str; // the spacecraft name

                cout << labelfile << " SPACECRAFT_NAME " << str << std::endl;

                if (str == "VIKING_ORBITER_1")
                    sc = "VIKING 1 ORBITER";
                if (str == "VIKING_ORBITER_2")
                    sc = "VIKING 2 ORBITER";
            }
            else if (str == "INSTRUMENT_NAME")
            {
                fin >> str; // the equals character
                fin >> str; // the instrument name

                cout << labelfile << " INSTRUMENT_NAME " << str << std::endl;

                if (str == "VISUAL_IMAGING_SUBSYSTEM_CAMERA_A")
                {
                    if (sc == "VIKING 1 ORBITER")
                        camera = "VO1_VISA";
                    else if (sc == "VIKING 2 ORBITER")
                        camera = "VO2_VISA";
                }
                else if (str == "VISUAL_IMAGING_SUBSYSTEM_CAMERA_B")
                {
                    if (sc == "VIKING 1 ORBITER")
                        camera = "VO1_VISB";
                    else if (sc == "VIKING 2 ORBITER")
...
/*
  ...
  file f056a93.INFO is created for the label file f056a93.imq_label.
...
int main(int argc, char** argv)
{
    ...
    for (unsigned int i=0; i<labelfiles.size(); ++i)
    {
        ...
        getCameraAndSc(labelfiles[i], camera, sc);
        ...
        getScOrientation(et, body, sc, camera, scposb, boredir, updir, frustum);
        if (failed_c())
            continue;
